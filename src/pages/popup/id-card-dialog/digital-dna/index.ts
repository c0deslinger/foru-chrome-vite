...
/**
 * Fetch Digital DNA data from authenticated API
 */
async function fetchAuthenticatedDigitalDna(accessToken: string): Promise<DnaData[]> {
  try {
    ...
    console.log(`🧬 Fetching authenticated DNA data for ID card`);
    ...
    if (dnaResponse.ok) {
      ...
      console.log("🧬 Authenticated DNA JSON for ID card", dnaJson);
      if (dnaJson.code === 200 && dnaJson.data && dnaJson.data.length > 0) {
        ...
        console.log(`🧬 Loaded ${digitalDnaData.length} authenticated DNA items for ID card`);
        return digitalDnaData;
      } else {
        console.log("🧬 No authenticated DNA data available for ID card");
      }
    ...
    console.error("🧬 Error fetching authenticated DNA data for ID card:", error);
  }
...
export async function drawDigitalDnaCard(
  ...
  if (accessToken) {
    try {
      dnaItems = await fetchAuthenticatedDigitalDna(accessToken);
    ...
    console.log('🧬 No access token provided for DNA');
  }

  // DNA grid (5x2) - same layout as badges
  const dnaSize = 40; // Same as badge size
  const dnaSpacing = 16; // Same as badge spacing
...
async function drawRealDnaItemWithTitle(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, dnaItem: DnaData): Promise<void> {
  ...
  // Draw DNA item background first
  ctx.fillStyle = '#2a2535';
  ctx.fillRect(x, y, size, size);

  // Draw DNA item border
